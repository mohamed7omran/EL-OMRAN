...
export default function ReportsPage() {
  ...
  useEffect(() => {
    const details = employees.map((employee) => {
      const employeeRecords = dailyRecords.filter((record) => record.employeeId === employee.id)
      const totalWage = employee.dailySalary * employee.attendanceDays
      const paidAmount = employeeRecords.reduce((sum, record) => sum + record.amount, 0)
    ...
    const expenses = details.reduce((sum, detail) => sum + detail.paidAmount, 0)
    setTotalExpenses(expenses)

    const pending = details.reduce((sum, detail) => sum + detail.pendingSalary, 0)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Total Pending Salaries</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold">${totalPendingSalaries.toFixed(2)}</p>
          </CardContent>
        </Card>
      </div>
      <Table>
